...
app.py のタブから `high_ui.render()` として呼ばれる。

このモジュールは UI だけを担当し、
  - 取得処理 -> modules/kabutan_high.py
  - DB 処理  -> modules/db.py
に完全に委譲している。将来 J-Quants / Yahoo Finance / TradingView を足す場合も、
同じ形の collect() を持つモジュールを modules/ に追加すれば、この画面は
ほぼ変更せずに使える。

■ タブとして描画する上での注意
・st.set_page_config は呼ばない（app.py が既に呼んでいる）。
・st.stop() は使わない。タブの中で呼ぶとアプリ全体が停止して
  他のタブまで描画されなくなるため、早期 return で抜ける。
・ウィジェットの key はすべて "high_" 始まりにする。app.py には
  多数のウィジェットがあり、ラベルが衝突すると状態を共有してしまうため。
"""
...
from modules import db, kabutan_high

# 将来のデータソース追加はここに足すだけでよい
# （collect(known_ids, known_dates, max_articles, deep_history, progress_cb) を満たすこと）
DATA_SOURCES = {
...
def _render_latest_week(streaks: pd.DataFrame) -> None:
    ...
    view = (
        latest.merge(streaks[["code", "current_streak", "count"]], on="code", how="left")
        .rename(
            columns={
                "code": "コード",
                "name": "銘柄",
...
def _render_search() -> str:
    ...
    with f1:
        period = st.selectbox(
            "期間指定", list(db.PERIOD_DAYS.keys()), index=4, key="high_period"
        )
    with f2:
        code_q = st.text_input("コード検索", placeholder="例: 8035", key="high_code_q")
    with f3:
        name_q = st.text_input("銘柄検索", placeholder="例: 東京エレク", key="high_name_q")
...
def _render_aggregations(streaks: pd.DataFrame, period: str) -> None:
    """集計（連続出現 / 出現回数 / 銘柄サマリ）。"""
    st.divider()
    st.subheader("集計")
    ...
    with tab_streak:
        ...
        only_active = st.checkbox(
            "連続中の銘柄だけ表示（連続 ≥ 2）", value=True, key="high_only_active"
        )
        ...
        st.dataframe(
            ...
            column_config={
                "連続": st.column_config.ProgressColumn(
                    ...
                    format="%d 週",
                    min_value=0,
                    max_value=int(streak_view["最長連続"].max())
                    if not streak_view.empty
                    else 1,
                ),
    ...
    with tab_rank:
        rank_view = db.ranking(period=period).rename(
            columns={"code": "コード", "name": "銘柄", "count": "出現回数"}
        )
    ...
    with tab_summary:
        summary_view = db.stock_summary(period=period).rename(
            columns={
                "code": "コード",
                "name": "銘柄",
                "first_seen": "初登場日",
...
def render() -> None:
    """上場来高値タブ全体を描画する。app.py のタブから呼ばれる入口。"""
    db.init_db()
    ...
    st.subheader("📈 上場来高値銘柄")
    st.caption(
        "株探「今週の【上場来高値銘柄】」を蓄積して、"
        "連続出現週数・出現回数・初登場日・直近登場日を集計します。"
    )
    ...
    if s["rows"] == 0:
        ...
        return  # タブ内なので st.stop() は使わない（アプリ全体が止まってしまう）

    # 連続週数は全期間で計算する（期間フィルタで切ると連続が途切れて見えてしまうため）
    streaks = db.streak_summary()

    _render_latest_week(streaks)
    period = _render_search()
    _render_aggregations(streaks, period)
